...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY, cellStartX+ UI.CellWidth, cellStartY+ UI.CellHeight);//*notcertain
	...
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	pWind->GetIntegerSize(w,h,cellNum);//*notcertain
	...
	pWind->DrawInteger(x, y, cellNum);//*notcertain
...
void Output::DrawLadder(const CellPosition & toCell, const CellPosition & fromCell) const
{
	...
	///TODO: Draw the cross horizontal lines of the ladder using the appropriate coordinates
		for(int i=0; y1 + UI.CellHeight / 2 + i * UI.CellHeight <y2;i++)
		{
			pWind->DrawLine(x12, y1+ UI.CellHeight/2 + i * UI.CellHeight, x34, y1 + UI.CellHeight / 2 + i * UI.CellHeight);
		}
