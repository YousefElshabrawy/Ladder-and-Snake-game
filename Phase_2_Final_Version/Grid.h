...
public:
	...
	void SetClipboard(CellPosition& card);  // A setter to be used in copy/cut (in order NOT to break class responsibilities)
	...
	Snake* GetNextSnake(const CellPosition& position);  // Gets a Pointer to the first Ladder after the passed "position"
	...
	void Load_cleanups();// We added this function once here because it is used many times by other classes
	bool IsOverLapping(GameObject* newObj)const;
	Player* GetPlayerByNum(int) const;
